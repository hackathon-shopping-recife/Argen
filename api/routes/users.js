  ...
  db.collection("cliente")
    ...
    .then(snapshot => {
      ...
      for (var item in horarioDevendas) {
        resultVendas.push({
          hora: horarioDevendas[item],
          quantidade: numeroDeVendas[item]
        });
      }

      resultVendas.sort(function(a, b) {
  ...
  db.collection("cliente")
    ...
    .then(snapshot => {
      snapshot.forEach(doc => {
        ...
        if (
          doc.data().genero == "Masculino" ||
          doc.data().genero == "masculino"
        ) {
          homens = homens + 1;
        } else if (
          doc.data().genero == "Feminino" ||
          doc.data().genero == "feminino"
        ) {
          mulheres = mulheres + 1;
        } else if (
          doc.data().genero == "Outros" ||
          doc.data().genero == "outros"
        ) {
          outros = outros + 1;
        } else {
